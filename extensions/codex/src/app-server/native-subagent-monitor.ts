...
class Monitor {
  ...
  // Keyed separately from `childStates` because OpenClaw is not the requester of a
  // descendant: it gets a relay claim only, and its lineage outlives `parentStates`.
  ...
  dispose(): void {
    ...
    // The closed client owns the process running these descendants, so their claims
    // die with it rather than pinning the route forever.
  ...
  registerParent(params: {
    ...
    // Only children spawned from here on inherit this attempt's hook command;
    // already-claimed children keep the route their own frozen command names.
  ...
  /** A worker's hook command is frozen at spawn, so its claim never moves route. */
  private attachChildNativeHookRelay(
    ...
    // Latch only a granted claim; latching a refusal would early-return every later
    // attach and leave this child unclaimed on the live successor route.
  ...
  /**
   * Descendants run their spawner's frozen hook command, so they claim the same route;
   * otherwise the last direct child settling fails their next PreToolUse closed.
   */
  private trackDescendantHookRelay(
    ...
    // Codex reports `idle` after every completed turn of a still-loaded thread, which
    // can be re-messaged at will; a descendant that never unloads pins the route
    // until `dispose`, which is cheaper than stranding a live worker.
...
/** Reads the thread ids one activity item announces as spawned from its sender thread. */
function readItemSpawnedThreads(
  ...
  if (method === "item/completed" && readString(item, "type") === "subAgentActivity") {
    // Stay kind-agnostic: a re-messaged sub-agent announces only `interacted`, so
    // gating on `started` would drop its task row, delivery, and relay claim.
