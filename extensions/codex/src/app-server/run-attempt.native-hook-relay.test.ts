...
describe("runCodexAppServerAttempt native hook relay", () => {
  ...
  it("installs only the adopted route's frozen hook surface on a cold resume", async () => {
    ...
    // A wider event set than the route was registered with: an approval-mode change
    // between turns resolves one, and a cold resume actually applies the overrides.
    const secondHarness = createResumeHarness();
    const secondParams = createParams(sessionFile, workspaceDir);
    secondParams.runId = "run-2";
    const secondRun = runCodexAppServerAttempt(secondParams, {
    ...
    // Installing it would fail closed on every permission request: adoption froze the
    // registration's allowed events, so the relay rejects the event the hook relays.
