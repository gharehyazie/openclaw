...
/** Codex announces a spawn only as a subAgentActivity item on the spawning thread. */
function spawned(
...
/** Runs one relay-enabled attempt to `turn/start` and reports the route its workers name. */
...
describe("Codex native hook relay lifecycle", () => {
  ...
  it("does not cache a route released inside its own constructor", () => {
    ...
    // The persisted generation makes the next attempt ask for this same relay id.
  ...
  it("ignores a superseded attempt's late release after adoption", async () => {
    ...
    // Attempt A's cleanup awaits several steps, so it lands after B adopted the route.
  ...
  it("re-registers a relay id that died under a live route", async () => {
    ...
    // Stands in for a lapsed-TTL prune or a bridge-record ownership transfer.
    registerNativeHookRelay({
      provider: "codex",
      relayId: first.relayId,
      sessionId: "session-1",
  ...
  it("detaches the cancelled attempt's failure sink from surviving workers", async () => {
    ...
    try {
      ...
      // Core freezes the sink at invocation entry, so a worker hook that started
      // unclaimed keeps reporting under its own turn after the next attempt adopts.
