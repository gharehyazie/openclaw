...
/**
 * One attempt's ref-counted view of a route that outlives it, so a holder drops its
 * own claim instead of unregistering or re-pointing the shared registration.
 */
...
export function createCodexNativeHookRelay(
  ...
  // Workers freeze their hook command at spawn, so one generation's route must
  // outlive its creating attempt: later attempts adopt it under the same relay id.
...
function registerCodexNativeHookRelay(
  ...
  options: {
    ...
    // Self-heal restores the route's frozen surface, not the adopting attempt's
    // policy view: workers' hook commands were installed against the former.
...
/** One attempt's claim on a route; the claim object itself is the release token. */
type CodexNativeHookRelayAttemptClaim = {
  readonly attempt: CodexNativeHookRelayAttempt;
  detachAbort: () => void;
};

/** The attempt-owned half of a registration; everything omitted is frozen per route. */
...
class CodexNativeHookRelayRoute {
  ...
  // Replaced only by `bindAttempt`'s self-heal, never by ordinary adoption.
  private relay: NativeHookRelayRegistrationHandle;
  private readonly lifetimeAbortController = new AbortController();
  // At most one: adoption retires the previous claim before binding the next.
  private attemptClaim: CodexNativeHookRelayAttemptClaim | undefined;
  private ttlMs: number;
  private hookTimeoutSec: number | undefined;
  // Frozen with allowedEvents so a self-heal restores the surface workers' hook
  // commands were installed against. Absent from the handle type, so not readable back.
  ...
  constructor(attempt: CodexNativeHookRelayAttempt, relayId: string) {
    ...
    this.relay = registerCodexNativeHookRelay(attempt, relayId, {
      ttlMs: this.ttlMs,
      // Worker-scoped: only route release cancels it; attempts layer their own on top.
      signal: this.lifetimeAbortController.signal,
      onPreToolUseFailure: (failure) =>
        this.reportUnclaimedPreToolUseFailure(attempt.runId, failure),
    });
    // Own the map entry before binding: an already-aborted attempt releases the route
    // synchronously, and that release must find this entry to evict it.
  ...
  /** Re-points a live route at the next attempt in place and hands it a lease. */
  adoptAttempt(attempt: CodexNativeHookRelayAttempt): CodexNativeHookRelayLease | undefined {
    // Fail closed: adopting a released route would silently unclaim every worker.
    if (this.released) {
      return undefined;
    }
    this.cancelPendingUnregister?.();
    this.cancelPendingUnregister = undefined;
    // Retire first so the superseded attempt's later `releaseParent` is a no-op.
    ...
    // Expiry was sized for the registering attempt; this one brings its own budget.
  ...
  private bindAttempt(attempt: CodexNativeHookRelayAttempt): CodexNativeHookRelayLease {
    ...
    const binding = {
      ...
      // Core snapshots both per invocation. Retiring the claim swaps them back to route
      // scope, keeping a retired attempt's abort and failure sink off surviving workers.
      signal: AbortSignal.any([this.lifetimeAbortController.signal, attempt.signal]),
      onPreToolUseFailure: attempt.onPreToolUseFailure,
    };
    // The registration can die under a live route (lapsed TTL, bridge takeover).
    // Re-register the same id rather than inherit a corpse that strands every worker
    // whose frozen hook command names it; the record replaced here is already gone.
    ...
    return {
      ...fields,
      // Read through: worker progress renews the route after the lease was handed out.
      get expiresAtMs() {
  ...
  /**
   * Codex never projects a native pre-tool failure as a tool item, so a failure no attempt
   * owns ends in silence unless recorded here. `runId` is frozen with the sink at bind time:
   * the live one belongs to whichever attempt adopted since, which never saw this tool call.
   */
  ...
  /** An aborted run drops its own claim; workers keep the route alive through theirs. */
  ...
  private renew(ttlMs?: number): void {
    ...
    // Never self-heals: only an attempt carries the routing facts a fresh
    // registration needs, so `bindAttempt` owns that repair.
  ...
  private acquireChild(childThreadIdInput: string): (() => void) | undefined {
    ...
    // A released route grants nothing, so the caller retries the live successor
    // instead of latching a corpse that leaves the worker permanently unclaimable.
  ...
  private releaseAttempt(
    ...
    // A superseded attempt's cleanup arrives several awaits later, by which time a
    // newer attempt may own the route, so a stale claim must not unclaim it.
  ...
  /**
   * Hands the registration back to the route: descendants inherit the hook command
   * but not the attempt's turn cancellation or failure sink.
   */
  ...
  private finalizeState(reason: string): void {
    ...
    this.clearRenewal();
    // Drop the listener without the retire rebind: the registration is gone.
...
// Routes are the registry's only writers, so this narrowing of the state module's
// structural entry type is exact; the state module stays route-type-free to avoid a cycle.
