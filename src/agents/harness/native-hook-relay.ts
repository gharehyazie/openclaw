...
export function registerNativeHookRelay(
  ...
  const handle: ActiveNativeHookRelayRegistrationHandle = {
    ...
    rebindAttempt: (binding) => {
      // Also reports liveness: a registration can die under a holder that still has
      // the handle, and only the holder knows whether re-registering is the repair.
...
/**
 * Swaps one attempt's routing facts in place. Mutation is the contract: the
 * registration object identity is the bridge's liveness token.
 */
...
export async function invokeNativeHookRelay(
  ...
  // Attempt facts freeze here; only relay-owned state below stays on the live registration.
